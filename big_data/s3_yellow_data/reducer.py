...
def perform_reduce(file_reducer: pd.DataFrame) -> pd.DataFrame:
    ...
    file_reducer = file_reducer.rename(columns={"month": "month", "payment_type": "payment_type", "tip_amount": "tips average amount"}).reset_index()
    file_reducer = file_reducer[(file_reducer['payment_type'] != 'Unknown')]
    return file_reducer
